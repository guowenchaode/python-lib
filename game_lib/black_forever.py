...
class DiabloWindowMonitor:
    def __init__(self, root):
        ...
        self.key_pos_records = (
            {}
        )  # 按键-坐标记录字典 {按键: (x_permil, y_permil)} 千分比
        ...
        self.loop_interval = 15  # 循环间隔时间（默认改为30秒）
        ...
        self.bubbles_visible = False  # 气泡是否显示
        ...
        self.mouse_monitor_thread = threading.Thread(
            target=self._monitor_mouse, daemon=True
        )
    ...
    def _init_ui(self):
        ...
        self.mouse_abs_label = ttk.Label(
            mouse_frame, text="绝对位置：(X: 0, Y: 0)", font=("微软雅黑", 10)
        )
        self.mouse_abs_label.pack(side=tk.LEFT, padx=10)

        self.mouse_rel_label = ttk.Label(
            mouse_frame, text="相对千分比：(X: ---, Y: ---)", font=("微软雅黑", 10)
        )
        ...
        # 循环间隔输入框
        ttk.Label(script_ctrl_frame, text="循环间隔(秒)：", font=("微软雅黑", 10)).pack(
            side=tk.LEFT, padx=5
        )
        self.interval_entry = ttk.Entry(
            script_ctrl_frame, width=8, font=("微软雅黑", 10)
        )
        ...
        self.script_status_label = ttk.Label(
            script_ctrl_frame, text="脚本状态：未加载", font=("微软雅黑", 10)
        )
    ...
    def _capture_main_window_screenshot(self):
        ...
        try:
            ...
            # 对主程序窗口进行截图
            # 方法1：精确截取主程序窗口（推荐）
            if hasattr(self.main_diablo_window, "left") and hasattr(
    ...
    def _check_main_window_foreground(self):
        ...
        try:
            ...
            if not foreground_window:
                return False
            return (
                foreground_window.title.strip() == self.main_diablo_window.title.strip()
            )
        except Exception:
            return False
    ...
    def _update_main_window_display(self):
        ...
        if self.main_diablo_window:
            ...
            self.main_window_label.config(
                text=f"当前主程序：{self.main_window_title[:20]}... (大小：{self.main_window_size[0]}x{self.main_window_size[1]})"
            )
            self.script_main_window_label.config(
                text=f"主程序：{self.main_window_title[:20]}...", foreground="green"
            )
        else:
    ...
    def _scroll_to_current_command(self):
        """自动滚动脚本表格到当前执行的命令行"""
        if self.script_current_index < 0 or self.script_current_index >= len(
            ...
            try:
                ...
                if self.main_diablo_window and self.main_window_size != (0, 0):
                    ...
                    if (
                        ...
                        <= mouse_x
                        <= self.main_diablo_window.left + self.main_window_size[0]
                        and self.main_diablo_window.top
                        <= mouse_y
                        <= self.main_diablo_window.top + self.main_window_size[1]
                    ):
                        # 计算千分比（保留3位小数，0.000-1.000）
                        rel_x_permil = round(
                            (
                                (mouse_x - self.main_diablo_window.left)
                        ...
                        rel_y_permil = round(
                            (
                                (mouse_y - self.main_diablo_window.top)
    ...
    def _on_key_press(self, event):
        ...
        # 仅工具前台且有主程序时记录
        if (
            not self.is_tool_foreground
            or not self.main_diablo_window
            or self.main_window_size == (0, 0)
            or self.script_running
        ):
            return
        ...
        x_permil = round(
            ((mouse_x - self.main_diablo_window.left) / self.main_window_size[0]), 3
        )
        y_permil = round(
            ((mouse_y - self.main_diablo_window.top) / self.main_window_size[1]), 3
        )
        ...
        if not windows_info:
            ...
            for win in windows_info:
                item = self.window_tree.insert(
                    "",
                    tk.END,
                    values=(win["title"], win["pos"], win["size"], win["status"]),
                )
                # 标记主程序行
                if (
                    self.main_diablo_window
                    and win["window_obj"].title == self.main_diablo_window.title
                ):
                    self.window_tree.item(item, tags=("main",))
                else:
                    self.window_tree.item(
                        item, tags=("active" if win["is_active"] else "inactive")
                    )
    ...
    def _update_mouse_info(self, abs_x, abs_y, rel_x_permil, rel_y_permil):
        ...
        if rel_x_permil != "--":
            ...
            self.mouse_rel_label.config(
                text=f"相对千分比：(X: {rel_x_show}, Y: {rel_y_show})"
            )
        else:
    ...
    def _update_status_label(self):
        ...
        if self.is_diablo_foreground:
            ...
            self.status_label.config(
                text="暗黑破坏神状态：后台运行", foreground="black"
            )
        ...
        if file_path:
            try:
                with open(file_path, "w", newline="", encoding="utf-8") as f:
                    ...
                    for key, (x_permil, y_permil) in sorted(
                        self.key_pos_records.items()
                    ):
                        ...
                        if len(row) == 3:
                            ...
                            if len(key) == 1:
                                try:
                                    ...
                                    # 确保千分比在0.000-1.000范围内
                                    x_permil = max(
                                        0.000, min(1.000, round(x_permil, 3))
                                    )
                                    y_permil = max(
                                        0.000, min(1.000, round(y_permil, 3))
                                    )
                                    self.key_pos_records[key] = (x_permil, y_permil)
                                    imported_count += 1
                                except ValueError:
        ...
        if file_path:
            ...
            try:
                ...
                self.script_status_label.config(
                    text=f"脚本状态：已加载({len(self.script_commands)}条命令，含系统倒计时)",
                    foreground="blue",
                )
            except Exception as e:
    ...
    def _pause_script(self):
        ...
        if self.script_paused:
            ...
            self.script_status_label.config(
                text="脚本状态：暂停中", foreground="orange"
            )
        ...
        while True:
            ...
            for index in range(len(self.script_commands)):
                try:
                    ...
                    print(
                        f"执行命令 {index+1}: 按键={cmd['key']}, X={cmd['x']}, Y={cmd['y']}, 来源={cmd['source']}"
                    )
                    cmd["status"] = "执行中"
